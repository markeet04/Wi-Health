/*
 * wi-netra Step 0 — RECEIVER (rx_csi_recv)
 *
 * Adapted from esp-csi/examples/get-started/csi_recv (Apache-2.0, Espressif).
 * Runs in promiscuous mode on a fixed channel, registers a CSI RX callback,
 * filters frames whose source MAC equals the tx_csi_send fixed MAC, and
 * prints one CSV row per CSI packet over USB serial at 921600 baud.
 *
 * No router, no STA association — pairs with tx_csi_send directly.
 *
 * CSV columns (ESP32-C6):
 *   type, seq, mac, rssi, rate, noise_floor, fft_gain, agc_gain,
 *   channel, local_timestamp, sig_len, rx_format, len, first_word, data
 *
 * "len"        = bytes in info->buf (I/Q pairs: 2 bytes per subcarrier).
 * "first_word" = info->first_word_invalid → 1 means the first 32-bit CSI
 *                word is invalid, useful for marking malformed frames.
 * "data"       = raw int8 I/Q array, quoted bracket-list.
 */
...
#define WIFI_CHANNEL            6     /* MUST match tx_csi_send */
/* ================================================ */

#define CONFIG_LESS_INTERFERENCE_CHANNEL   WIFI_CHANNEL

/* HT20 everywhere — matches the TX side. */
...
static const char *TAG = "rx_csi_recv";
...
static void wifi_esp_now_init(esp_now_peer_info_t peer)
{
    ...
    /* ESP32-S3: esp_now_set_peer_rate_config() is declared but the per-peer
     * rate-control backing store is not implemented in the S3 Wi-Fi driver,
     * so calling it faults with LoadProhibited. Fall back to the (deprecated
     * but functional) global ESP-NOW rate API. Not strictly required on the
     * RX side — this side only receives — but kept for symmetry with TX. */
...
static void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info)
{
    ...
    /* Only keep frames from our paired TX. */
    ...
    /* LOGD (not LOGI) — must NOT spill into the CSV stream. */
    ESP_LOGD(TAG, "compensate_gain %f, agc_gain %d, fft_gain %d", compensate_gain, agc_gain, fft_gain);
#endif

    uint32_t rx_id = *(uint32_t *)(info->payload + 15);  /* TX-side counter embedded in ESP-NOW payload */
...
static void wifi_csi_init(void)
{
    ...
    wifi_csi_config_t csi_config = {
        .enable                   = true,
        .acquire_csi_legacy       = true,   /* LLTF */
        .acquire_csi_force_lltf   = CSI_FORCE_LLTF,
        .acquire_csi_ht20         = true,   /* HT-LTF in HT20 frames */
        .acquire_csi_ht40         = false,  /* HT20-only deployment */
    ...
    /* ESP32-C6 in HT20: enable both legacy (LLTF) and HT20 (HT-LTF) capture
     * so we get the full subcarrier set. HT40 disabled — we run HT20-only. */
    wifi_csi_config_t csi_config = {
        .enable                 = true,
